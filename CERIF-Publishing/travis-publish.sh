...
then
	...
	then
		...
		rsync -e 'ssh -i .ssh/cerif.eurocris.org.key -l jdvorak' -crz --delay-updates --delete-after --itemize-changes "$@" cerif.eurocris.org:/data-eurocris/documentRoot/wwwcerif/
	else
